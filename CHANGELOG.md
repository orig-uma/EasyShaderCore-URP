...
- **顔 SDF の縦スイープ出力**（`EasyPbrFaceSdfBaker.Settings.bakeVertical`）。`pack16` と併用すると、右光スイープ（R×256+G）に加えて**縦スイープ 2 本**（上: 正面 → 真上 → 背面 / 下: 正面 → 真下 → 背面）を B / A に 8bit ずつ焼く。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライト・下からの光で鼻下・唇・顎裏が「正面光」と読まれて明るいまま残っていた。縦には左右対称が無いので上下 2 本要り、8bit でも**角度に線形**（th/180°）で格納するので刻みは一様 0.7°（cos 空間の 8bit は正面付近が 5° 刻み）。縦は反転しない。X Axis Tilt は横だけに掛かる。距離場ブレンド・ブラーはチャンネルごとに float 域で掛ける。既定 OFF。読み方は `Documentation~/FACE_SDF_BAKING.md` §1.2。

## [0.3.5] - 2026-09-18

### Added

- **ベイクの材質グループ**（`EasyPbrBakeCore.GroupScope(Material[], groupName)` / `LastGroupOverlap`）。`using` で囲んだ間の各ベイカーは、グループのどれかの材質を使うメッシュ・サブメッシュをまとめて 1 枚に焼き、保存名に材質名ではなくグループ名を使い、スロットがあれば全材質に割り当てる。同じ UV アトラスを共有する材質が、焼いたマップも 1 枚を共有できるようにするためのもの（材質ごとに焼くと各テクスチャの大半が空白のまま材質の数だけ増える）。グループ内の別の材質どうしで UV が重なった割合を `LastGroupOverlap` に入れ、2% を超えると警告する。各ベイカーの呼び出し口は変えていない。後方互換。

## [0.3.4] - 2026-09-16

### Changed

- **Glitter の近傍探索を 1 セル 1 ハッシュにした**（`Hash24` を `Common_Math.hlsl` に追加、`PrepareGlitter` が使う）。以前はセルごとに `Hash21` を 3 回、最寄りセルにさらに 2 回で計 29 回呼んでいた。4 値を一度に返す `Hash24` で 9 回にし、間引きはその 4 値から作る。**粒の配置は以前と変わる**（乱数列が違うため。密度・大きさの分布は同じ）。Doll / Idol 共通。
