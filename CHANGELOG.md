...
- **顔 SDF の 16bit 2ch 出力**（`EasyPbrFaceSdfBaker.Settings.pack16Vertical`）。`pack16` と併用すると、右光スイープ（R×256+G）に加えて**上光スイープ**（正面 → 真上 → 背面）を同じ反転規約で B×256+A に焼く。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライトのとき鼻下・唇・顎裏が「正面光」と読まれて明るいまま残っていた。縦は左右対称の前提が要らないのでミラー無し、X Axis Tilt は横だけに掛かる。距離場ブレンド・ブラーはチャンネルごとに float 域で掛ける。既定 OFF（従来の 1ch 出力のまま）。読み方は `Documentation~/FACE_SDF_BAKING.md` §1.2。
